    ...
    public static float[,] ElasticEJ(float[,] input, int outputSize, float alpha)
    {
        ...
        for (int l = 0; l < outputSize; l++)
        {
           ...
           //         displacementField[i, j].Normalize();
            ...
            for (int i = 0; i < displacementField.GetLength(0); i++)
                for (int j = 0; j < displacementField.GetLength(1); j++)
                    displacementField[i, j] = new Vector2(0, 0);

            for (int i = 0; i < displacementField.GetLength(0); i++)
                for (int j = 0; j < displacementField.GetLength(1); j++)
                    displacementField[i, j] = new Vector2(1f, 0);
            ...
            for (int i = 0; i < displacementField.GetLength(0); i++)
            {
                for (int j = 0; j < displacementField.GetLength(1); j++)
                {
                    ...
                    */
                    ...
                    if (0 <= j + (int)dx && j + (int)dx < 28)
                        y1 = j + (int)dx;
                    else
                        y1 = 0;

                    if (0 <= j + (int)dx + xOffSet && j + (int)dx + xOffSet < 28)
                        y2 = j + (int)dx + xOffSet;
                    else
                        y2 = 0;
                    ...
                    /*
                     ...
                     y1 = j + (int)dx >= 0 && j + (int)dx < 28 ? j + (int)dx : 0;
                     y2 = j + (int)dx + xOffSet >= 0 && j + (int)dx + xOffSet < 28 ? j + (int)dx + xOffSet : 0;
                     x1 = i + (int)dy >= 0 && i + (int)dy < 28 ? i + (int)dy : 0;
                     x2 = i + (int)dy + yOffSet >= 0 && i + (int)dy + yOffSet < 28 ? i + (int)dy + yOffSet : 0;
                     */
